...
public:
	...
	{
		// temp pi
		double const temp_pi=3.1415926535897932384626433832795f;
		
		// temp coef vars
		double alpha,a0,a1,a2,b0,b1,b2;

		double cfrequency = 2.0 * pow(10.0, ((frequency*3.0) + 1.0));

		// peaking, lowshelf and hishelf
		if(type>=6)
		{
			double const A		=	pow(10.0,(db_gain/40.0));
			double const omega	=	2.0*temp_pi*cfrequency/sample_rate;
		...
		{
			// other filters
			double const omega	=	(double)2.0*temp_pi*cfrequency/sample_rate;
			double const tsin	=	sin(omega);
			double const tcos	=	cos(omega);
