...
FreqAnalyzer::Freq (MasterAndCommander *m)
    : master(m),
      gridX(32),
      gridY(32),
      offsetX(32),
      ...
      fs(0)
{
    ...
    master->registerFreqAnalyzer(this);
}
...
void FreqAnalyzer::paint (Graphics& g)
{
    int width = getWidth();
...
void FreqAnalyzer::resized()
{
}


void FreqAnalyzer::init(int gx, int gy, int ox, int oy,
...
void FreqAnalyzer::setLevel(float *data)
{
    for (int i = 0; i < numLevel; i++)
        level[i] = data[i];
...
void FreqAnalyzer::setImpulse(float *data)
{
    for (int i = 0; i < 2 * numLevel; i++)
        impulse[i] = data[i];
